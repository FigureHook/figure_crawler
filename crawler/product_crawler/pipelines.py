        ...
        with db(os.environ["DB_URL"], echo=False) as d:
            ...
            if len(product.release_infos):
                if spider.name == "gsc_product":
                    ...
                    product.release_infos[0].announced_at = date

                if item["order_period"]:
                    product.release_infos[-1].order_period_start = item["order_period"].start
                    product.release_infos[-1].order_period_end = item["order_period"].end
